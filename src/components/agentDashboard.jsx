...
function AgentDashboard() {
  ...
  // Missing state used throughout this file
  ...
  // root class selector (fixes ReferenceError)
  ...
  return (
    <div className="relative inline-block">
      <button
        ...
        className="flex items-center gap-2 rounded-lg bg-white dark:bg-background-dark border border-border-light dark:border-border-dark h-12 px-3"
        aria-expanded={isOpen}
      >
        <span className="text-sm">{label}</span>
        <span className="material-symbols-outlined text-inactive">
      ...
      {isOpen && (
        <ul
          ref={dropdownRef}
          className="absolute mt-2 right-0 w-48 bg-white dark:bg-background-dark border border-border-light dark:border-border-dark rounded-lg shadow-lg z-50"
        >
          {options.map((opt) => (
            <li key={opt}>
              <button
                ...
                className="w-full text-left px-4 py-2 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
              >
                {opt}
              </button>
            </li>
...
function UsageStats({ topPerformers, agentCount }) {
  return (
    <aside className="bg-white dark:bg-background-dark border border-border-light dark:border-border-dark rounded-lg p-4">
      <h3 className="text-lg font-semibold mb-3">Usage & Top Agents</h3>
      <p className="text-sm text-inactive mb-3">{agentCount} agents</p>
      <ul className="space-y-3">
        {topPerformers.map((a) => (
          <li key={a._id} className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-md bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-sm font-medium">
              {(String(a.AgentName || a.name || "A")[0] || "A").toUpperCase()}
            </div>
            <div>
              <div className="text-sm font-medium">{a.AgentName || a.name || "Unnamed Agent"}</div>
              <div className="text-xs text-inactive">{String(a.status || a.Status || "active")}</div>
            </div>
          </li>
        ))}
      </ul>
